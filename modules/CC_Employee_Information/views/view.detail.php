...
class CC_Employee_InformationViewDetail extends ViewDetail
{
     ...
     */
    public function CC_Employee_InformationViewDetail()
    {
    ...
    public function HideSubPanels(){
        echo '<script>
               $( document ).ready(function() {
                   $(\'#subpanel_title_cc_recruitment_request_cc_employee_information\').parent().parent().hide();
               });
              </script>';
    }
    ...
    public function populateInvoiceTemplates()
    {
        ...
        $sql = "SELECT id, name FROM aos_pdf_templates WHERE deleted = 0 AND type='CC_Employee_Information' AND active = 1";
    ...
    public function displayPopupHtml()
    {
        ...
        if ($templates) {
            ...
            echo '<input type="hidden" name="templateID" value="" />
				<input type="hidden" name="action" value="CreateResume" />
    ...
    public function displayProfileMatcher() {
        ...
        $profileRateArea = new ProfileRatingArea(0,0,0);
        $smarty->assign('ratingResults', $profileRateArea->RatingArea());
