{literal}
    <script type="text/javascript" src='custom/include/generic/javascript/datatables/jquery.dataTables.js'></script>
    <script type="text/javascript" src="custom/include/generic/javascript/toastr/toastr.min.js"></script>
<link href="custom/include/generic/css/jquery.dataTables.css" rel="stylesheet" type="text/css">
<link href="custom/include/generic/css/toastr.css" rel="stylesheet" type="text/css">
<link href="modules/CT_daily_status_report/css/list.css" rel="stylesheet" type="text/css"/>
    <div class="moduleTitle"><h2 class="module-title-text">Daily Status Report</h2></div>
    <div class="clear"></div>
<div class="date_range_selector">
    <div>
        <div class="col-lg-3 col-md-6 col-xs-6 col-print-12">
            <div class="form-group">
                <label for="activity_from">Select a Project</label><br/>
                <span class="project-select">
                <div class="row">
                  <div style="width: 100%">
                    {/literal}
                      {html_options name="selected_project" id="selected_project" selected=$actual_project options=$project_list}
                      {literal}
                  </div>
                </div>
            </span>
            </div>
        </div>
        <div class="col-lg-2 col-md-6 col-xs-6 col-print-6">
            <div class="form-group">
                <label for="activity_from">From Date</label><br/>
                <span class="dateTime">
                    <div class="row">
                      <div class="col-md-9 col-xs-9">
                        <input class="date_input" autocomplete="off" type="text" name="activity_from" id="activity_from"
                               style="width: 100%; height: 34px" maxlength="10" value="{/literal}{$activity_from}{literal}"/>
                      </div>
                      <div class="col-md-3 col-xs-3">
                        <button type="button" id="activity_from_on_create_trigger" class="btn btn-danger" style="float: right"
                                onclick="return false;"><span class="suitepicon suitepicon-module-calendar"
                                                              alt="Enter Date"></span></button>
                      </div>
                    </div>
                </span>
            </div>
        </div>
        <div class="col-lg-2 col-md-6 col-xs-6 col-print-6">
            <div class="form-group">
                <label for="activity_to">To Date</label><br/>
                <span class="select">
                    <div class="row">
                      <div class="col-md-9 col-xs-9">
                        <input class="date_input" autocomplete="off" type="text" name="activity_to" id="activity_to"
                               style="width: 100%; height: 34px" maxlength="10" value="{/literal}{$activity_to}{literal}"/>
                      </div>
                      <div class="col-md-3 col-xs-3">
                        <button type="button" id="activity_to_on_create_trigger" class="btn btn-danger" style="float: right"
                                onclick="return false;"><span class="suitepicon suitepicon-module-calendar"
                                                              alt="Enter Date"></span></button>
                      </div>
                    </div>
                </span>
            </div>
        </div>
        <div class="col-lg-2 col-md-6 col-xs-6">
            <div class="form-group">
                <label for="search_action"></label><br/>
                <span class="dateTime">
                    <div class="row">
                      <div class="col-md-2 col-xs-2" style="padding: 5px;">
                          <input id="search_action" type="button" name="Search" value="Search" class="button">
                      </div>
                    </div>
                </span>
            </div>
        </div>
    </div>
</div>
    <div class="clear"></div>
{/literal}
<div class="clear"></div>
<div id="summaryDailyStatusReportDiv" class="col-lg-12 col-xs-12 col-print-12">
    <div id="warningDailyStatusReportDiv" class="col-lg-2 col-xs-12 col-print-6">
        <h4>Teammates Delayed</h4>
        <div class="dailyreportimg floatLeft"><img src="modules/CT_daily_status_report/tpls/imgs/alert-delayed.svg">
        </div>
        <div class="floatRight" >
            <ul class="report_scrollable">
                {foreach from=$EMPLOYEE_DELEAYED item=delay_employee key=keyA}
                    <li><a href="index.php?module=CC_Employee_Information&offset=1&return_module=CC_Employee_Information&action=DetailView&record={$keyA}">{$delay_employee}</a></li>
                {/foreach}
            </ul>
        </div>
    </div>
    <div id="blockDailyStatusReportDiv" class="col-lg-2 col-xs-12 col-print-6">
        <h4>Teammates Block</h4>
        <div class="dailyreportimg floatLeft"><img src="modules/CT_daily_status_report/tpls/imgs/alert-block.svg"></div>
        <div class="floatRight" >
            <ul class="report_scrollable">
                {foreach from=$EMPLOYEE_BLOCK item=block_employee key=keyB}
                    <li><a href="index.php?module=CC_Employee_Information&offset=1&return_module=CC_Employee_Information&action=DetailView&record={$keyB}">{$block_employee}</a></li>
                {/foreach}
            </ul>
        </div>
    </div>
    <div id="missingDailyStatusReportDiv" class="col-lg-2 col-xs-12 col-print-6">
        <h4>Missing Reports <span>({$activity_from})</span></h4>
        <div class="dailyreportimg floatLeft"><img src="modules/CT_daily_status_report/tpls/imgs/alert-missing.svg"></div>
        <div class="floatRight" >
            <ul class="report_scrollable">
                {foreach from=$EMPLOYEE_MISSING item=missing_employee key=keyC}
                    <li><a href="index.php?module=CC_Employee_Information&offset=1&return_module=CC_Employee_Information&action=DetailView&record={$keyB}">{$missing_employee}</a></li>
                {/foreach}
            </ul>
        </div>
    </div>
    <div id="meterDailyStatusReportDiv" class="col-lg-6 col-xs-12 col-print-12">
        <div class="mood mood-fantastic col-lg-1 col-print-1 col-xs-12">
            <div class="mood-title"><h4>Fantastic</h4></div>
            <div class="dailyreportimg floatLeft"><img
                        src="modules/CT_daily_status_report/tpls/imgs/mood-fantastic.svg"></div>
            <div class="moodvalue floatRight">{$MOOD_FANTASTIC}</div>
        </div>
        <div class="mood mood-great col-lg-1 col-print-1 col-xs-12">
            <div class="mood-title"><h4>Great</h4></div>
            <div class="dailyreportimg floatLeft"><img src="modules/CT_daily_status_report/tpls/imgs/mood-great.svg">
            </div>
            <div class="moodvalue floatRight">{$MOOD_GREAT}</div>
        </div>
        <div class="mood mood-okey col-lg-1 col-print-1 col-xs-12">
            <div class="mood-title"><h4>Okay</h4></div>
            <div class="dailyreportimg floatLeft"><img src="modules/CT_daily_status_report/tpls/imgs/mood-okey.svg">
            </div>
            <div class="moodvalue floatRight">{$MOOD_OKAY}</div>
        </div>
        <div class="mood mood-down col-lg-1 col-print-1 col-xs-12">
            <div class="mood-title"><h4>Down</h4></div>
            <div class="dailyreportimg floatLeft"><img src="modules/CT_daily_status_report/tpls/imgs/mood-down.svg">
            </div>
            <div class="moodvalue floatRight">{$MOOD_DOWN}</div>
        </div>
        <div class="mood mood-depressed col-lg-1 col-print-1 col-xs-12">
            <div class="mood-title"><h4>Depressed</h4></div>
            <div class="dailyreportimg floatLeft"><img
                        src="modules/CT_daily_status_report/tpls/imgs/mood-depressed.svg"></div>
            <div class="moodvalue floatRight">{$MOOD_DEPRESSED}</div>
        </div>
        <div class="mood mood-angry col-lg-1 col-print-1 col-xs-12">
            <div class="mood-title"><h4>Angry</h4></div>
            <div class="dailyreportimg floatLeft"><img src="modules/CT_daily_status_report/tpls/imgs/mood-angry.svg">
            </div>
            <div class="moodvalue floatRight">{$MOOD_ANGRY}</div>
        </div>
    </div>
</div>
<div class="clear"></div>
<br>
<div class="clear"></div>
<div id="mainDailyStatusReportDiv" class="col-lg-12 col-xs-12">
    <div class="card">
        <div class="card-body">
            <div class="table-responsive">
                <table id="dataTableDailyStatusReport" class="table table-bordered table-striped table-hover"
                       style="width:100%">
                    <thead>
                    <tr>
                        <th>Id</th>
                        <th>Employee Id</th>
                        <th>Employee Name</th>
                        <th>Project Id</th>
                        <th>Project Name</th>
                        <th>Date Reported</th>
                        <th>Yesterday</th>
                        <th>Today</th>
                        <th>Reason</th>
                        <th>Eta</th>
                        <th>Blockers 30 days</th>
                        <th>Schedule</th>
                        <th>Delays 30 days</th>
                        <th>Mood</th>
                    </tr>
                    </thead>
                    <tbody></tbody>
                </table>
            </div>
        </div>
    </div>
</div>
<div class="clear"></div>
<div id="noData" class="empty-state" style="display: none;">
    <div class="empty-state__content">
        <div class="empty-state__icon">
            <img src="data:image/png;base64,"
                 alt="">
        </div>
        <div class="empty-state__message">No records have been found, try another date range.</div>
        <div class="empty-state__help"></div>
    </div>
</div>
<script type="text/javascript" src="modules/CT_daily_status_report/js/list.js"></script>
