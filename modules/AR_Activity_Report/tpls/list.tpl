{literal}
<script type="text/javascript" src='custom/include/generic/javascript/datatables/jquery.dataTables.js'></script>
<script type="text/javascript" src="custom/include/generic/javascript/toastr/toastr.min.js"></script>
<link href="custom/include/generic/css/jquery.dataTables.css" rel="stylesheet" type="text/css">
<link href="custom/include/generic/css/toastr.css" rel="stylesheet" type="text/css">
<link href="modules/AR_Activity_Report/css/edit.css" rel="stylesheet" type="text/css" />
<link href="modules/AR_Activity_Report/css/list.css" rel="stylesheet" type="text/css" />
<div class="moduleTitle"><h2 class="module-title-text">&nbsp;Activity Report</h2></div>
<div class="clear"></div>
<div>
    <div class="col-lg-2 col-md-6 col-xs-6">
        <div class="form-group">
            <label for="date_from">From Date</label><br/>
            <span class="dateTime">
                <div class="row">
                  <div class="col-md-9 col-xs-9">
                    <input class="date_input" autocomplete="off" type="text" name="date_from" id="date_from"
                           style="width: 100%; height: 34px" maxlength="10" value="{/literal}{$activity_from}{literal}"/>
                  </div>
                  <div class="col-md-3 col-xs-3">
                    <button type="button" id="date_from_on_create_trigger" class="btn btn-danger" style="float: right"
                            onclick="return false;"><span class="suitepicon suitepicon-module-calendar" alt="Enter Date"></span></button>
                  </div>
                </div>
            </span>
        </div>
    </div>
    <div class="col-lg-2 col-md-6 col-xs-6">
        <div class="form-group">
            <label for="date_to">To Date</label><br/>
            <span class="select">
                <div class="row">
                  <div class="col-md-9 col-xs-9">
                    <input class="date_input" autocomplete="off" type="text" name="date_to" id="date_to"
                           style="width: 100%; height: 34px" maxlength="10" value="{/literal}{$activity_to}{literal}"/>
                  </div>
                  <div class="col-md-3 col-xs-3">
                    <button type="button" id="date_to_on_create_trigger" class="btn btn-danger" style="float: right"
                            onclick="return false;"><span class="suitepicon suitepicon-module-calendar" alt="Enter Date"></span></button>
                  </div>
                </div>
            </span>
        </div>
    </div>
        <div class="col-lg-2 col-md-6 col-xs-6">
        <div class="form-group">
            <label for="related_to">Related To:</label><br/>
            <span class="select">
                <div class="row">
                     <select class="datetimecombo_time related" size="1" id="related_to" tabindex="0" >
                        <option selected=""></option>
                        {/literal}
                          {foreach from=$RELATES item=relate}
                              <option value="{$relate}">{$relate}</option>
                          {/foreach}
                        {literal}
                     </select>
                </div>
            </span>
        </div>
    </div>
    <div class="col-lg-2 col-md-6 col-xs-6">
        <div class="form-group">
            <label for="search_action"></label><br/>
            <span class="dateTime">
                <div class="row">
                  <div class="col-md-2 col-xs-2" style="padding: 5px;">
                      <input id="search_action" type="button" name="Search" value="Search" class="button">
                  </div>
                </div>
            </span>
        </div>
    </div>
    <br>
</div>
{/literal}
<div class="clear"></div>
<div id="mainActivityDiv" class="col-lg-12 col-xs-12">
    <div class="card">    
    <div class="card-body">
    <div class="table-responsive" >
      <table id="dataTableActivityReport" class="table table-bordered table-striped table-hover" style="width:100%">
        <thead>
        <tr>
          <th>&nbsp;</th>
          <th>Subject</th>
          <th>Agenda</th>
          <th>Type</th>
          <th>Related To</th>
          <th>Start Date</th>
          <th>Status</th>
          <th>Duration</th>
          <th>Actions</th>
        </tr>
        </thead>
        <tbody></tbody>
      </table>
    </div>
    </div>
  </div>
</div>
<div class="clear"></div>
<div id="noData" class="empty-state" style="display: none;">
    <div class="empty-state__content">
        <div class="empty-state__icon">
            <img src="data:image/png;base64," alt="">
        </div>
        <div class="empty-state__message">No records have been found, try another date range.</div>
        <div class="empty-state__help"></div>
    </div>
</div>
{literal}
<script type="text/template" data-template="activity_report_action_template">
/*
    <div class="activity_report_actions">
        <span class="ui-button-icon-primary ui-icon ui-icon-play ar-icon ar-iconRunning${status}" onclick="activity_report_action('${id}','play',${accumulated_time})"></span>
        <span class="ui-button-icon-primary ui-icon ui-icon-pause ar-icon ar-iconPaused${status}" onclick="activity_report_action('${id}','pause',${accumulated_time})"></span>
        <span class="ui-button-icon-primary ui-icon ui-icon-stop ar-icon ar-iconCompleted${status}" onclick="activity_report_action('${id}','stop',${accumulated_time})"></span>
        <div class="tooltip" style="opacity : 1; display: contents">
            <span class="tooltip-content">${partName}</span>
            <span class="ui-button-icon-primary ui-icon ui-icon-info" style="float: right;"></span>
        </div>
    </div>
*/
</script>
<script type="text/javascript">
    function createActionColumn(props) {
        return function (tok, i) {
            return (i % 2) ? props[tok] : tok;
        };
    }

    function postUpdateAction(element_id,action){
        $.ajax({
            type: "POST",
            url: 'index.php?entryPoint=ActivityReportEntryPoint',
            data: {
                action: 'UpdateActivityStatus',
                elementId:element_id,
                activity_action : action,
            },
            success: function(resp) {
                $("#search_action").trigger( "click" );
                const result_object = JSON.parse(resp);
                if(result_object?.result){
                    toastr.success(result_object?.msg, 'Success');
                } else {
                    toastr.error(result_object?.msg, 'Error');
                }
            }
        });

    }

    function activity_report_action(element_id,action,accumulated_time){
        if((action=='stop') && (accumulated_time/60)>8){
            toastr.warning("This activity report longer than 8 hours the value will be set to 8 by default", 'Oops!');
        }
        postUpdateAction(element_id,action);
    }

    $(document).ready(function(){
        toastr.options = {
            "positionClass": "toast-bottom-right",
        }

        $('#MassAssign_SecurityGroups').hide();

        const action_template = $('script[data-template="activity_report_action_template"]').text()
            .replace('/*', '').replace('*/', '').split(/\$\{(.+?)\}/g);

        const isValidDate = (dateString) => {
            const regEx = /^\d{4}-\d{2}-\d{2}$/;
            if(!dateString.match(regEx)) return false;  // Invalid format
            const d = new Date(dateString);
            const dNum = d.getTime();
            if(!dNum && dNum !== 0) return false; // NaN value, Invalid date
            return d.toISOString().slice(0,10) === dateString;
        }

        const validateInput = (e) =>{
            if(!isValidDate($(e).val())) {
                $(e).css('border', 'solid 2px red');
                $('#search_action').prop('disabled', true)
                return false;
            } else {
                $(e).css('border', '1px solid #090a0a');
                $('#search_action').prop('disabled', false)
                return true;
            }
        }
        const validateIt = () =>{
            setTimeout(function () {
                $('.date_input').each(function() {
                    let result = validateInput("#"+this.id);
                    if(!result) {
                        return false;
                    }
                });
            },100);
        }

        Calendar.setup ({
            inputField : "date_from",
            ifFormat : "%Y-%m-%d",
            daFormat : "%Y-%m-%d",
            button : "date_from_on_create_trigger",
            singleClick : true,
            dateStr : "",
            startWeekday: 0,
            step : 1,
            weekNumbers:false
        });

          const inputElementFrom = document.getElementById('date_from');
          YAHOO.util.Event._addListener(inputElementFrom, "change",  validateIt );

        Calendar.setup ({
            inputField : "date_to",
            ifFormat : "%Y-%m-%d",
            daFormat : "%Y-%m-%d",
            button : "date_to_on_create_trigger",
            singleClick : true,
            dateStr : "",
            startWeekday: 0,
            step : 1,
            weekNumbers:false
        });

        const inputElementTo = document.getElementById('date_to');
            YAHOO.util.Event._addListener(inputElementTo, "change",  validateIt );

            $('.date_input').keydown(function(e) {
                if ( e.keyCode >= 37 && e.keyCode <= 40 ||
                     e.keyCode >= 46 && e.keyCode <= 57 || e.keyCode == 189 ||  e.keyCode == 8 ||  e.keyCode == 9) {
                    return true;
                } else {
                    return false;
                }
            });

            $('.date_input').focusout(function() {
                validateInput("#"+this.id)
            }).trigger("focusout");

        function updateDatesRange(){
            $.cookie('activity_date_from', $("#date_from").val());
            $.cookie('activity_date_to', $("#date_to").val());
            $.cookie('activity_related', $("#related_to").val());
        }

        updateDatesRange();

        tablaActivityReport = $("#dataTableActivityReport").DataTable({
            "responsive": true,
            "ajax": {
                "url": 'index.php?entryPoint=ActivityReportEntryPoint',
                "type": "POST",
                'data': function (d) {
                    d.action = "get";
                }
            },
            "paging": true,
            "order": [[0, "desc"]],
            "info": true,
            "filter": true,
            "columns": [
                {orderable: false, searchable: false, name: "id"},
                {orderable: true, searchable: true, name: "m.name" },
                {orderable: true, searchable: true, name: "m.description" },
                {orderable: true, searchable: false, name: "type" },
                {orderable: true, searchable: false, name: "ar.parent_type" },
                {orderable: true, searchable: false, name: "m.date_start" },
                {orderable: true, searchable: false, name: "status" },
                {orderable: true, searchable: false, name: "result" },
                {orderable: false, searchable: false, name: "actions" },
            ],
            "columnDefs": [{
                "targets": [0],
                "sortable": false,
                "data": "id",
                "className": "td_delete_profile",
                "render": function(data, type, row){
                    let url = `index.php?module=${row.object_name}&offset=1&return_module=${row.object_name}&action=EditView&record=${row.id}`;
                    let edit = (row.canEdit == 0) ? '': `<input type=hidden value=${row.id} ><a href="${url}" target="_self" class="edit-link"><span class="suitepicon suitepicon-action-edit"></span></a>`
                    return edit;
                }
            },{
                "targets": [1],
                "data": "name",
                "className": "",
                "render": function(data, type, row){
                    return `<div>${row.subject}</div>`;
                }
            },{
                "targets": [2],
                "data": "id",
                "className": "",
                "render": function(data, type, row){
                    return `<div>${row.agenda}</div>`;
                }
            },{
                "targets": [3],
                "data": "id",
                "className": "",
                "render": function(data, type, row){
                    return `<div>${row.typeAr}</div>`;
                }
            },{
                "targets": [4],
                "data": "id",
                "className": "",
                "render": function(data, type, row){
                    return `<div>${row.parent_type}</div>`;
                }
            },{
                "targets": [5],
                "data": "id",
                "className": "",
                "render": function(data, type, row){
                    return `<div>${row.date_start}</div>`;
                }
            },{
                "targets": [6],
                "data": "id",
                "className": "",
                "render": function(data, type, row){
                    let extra_time = '';
                    let time_text = '';
                    if(row.status=='Paused'){
                        extra_time = `Accumulated Time: ${row.accumulated_days}days ${row.accumulated_hours}hours ${row.accumulated_minutes}minutes`
                    }
                    if(row.status=='Running'){
                        extra_time = `Running Time: ${row.running_days}days ${row.running_hours}hours ${row.running_minutes}minutes since last update`
                    }
                    if(extra_time.length>0){
                        time_text = `<a href="#" title="${extra_time}"><span class="suitepicon suitepicon-action-info"></span></a>`;
                    }

                    return `<div class="ar-col-status-${row.status}">${row.status}${time_text}</div>`;
                }
            },{
                "targets": [7],
                "data": "id",
                "className": "",
                "render": function(data, type, row){
                    return `<div>${row.duration}</div>`;
                }
            },{
                "targets": [8],
                "data": "id",
                "className": "",
                "render": function (data, type, row) {
                    let currentActionTpl = action_template.map(createActionColumn(row)).join('');
                    return currentActionTpl;
                }
            }],
            "drawCallback": function (settings, json) {
                /*if(settings.aoData.length ===0){
                    $('#mainActivityDiv').hide();
                    $('#noData').show();
                } else {
                    $('#mainActivityDiv').show();
                    $('#noData').hide();
                }*/
            },
            "processing": true,
            "serverSide": true,
            "lengthMenu": [[10, 25, 50, 100], [10, 25, 50, 100]],
            "pageLength": 25,
            "displayLength": 25,
        });

        $("#search_action").click(function () {
            updateDatesRange();
            if (typeof tablaActivityReport !== 'undefined') {
                tablaActivityReport.ajax.reload();
            }
        });

    });
</script>
{/literal}
